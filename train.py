...
def train(args):
    ...
    elif args.phase in ['train_post', 'train_post_manualmask']:
    ...
    if args.inpaint_type.startswith('polynomial') and args.phase == 'train_post_manualmask':
        order_all = 12
        if args.mode3d:
            dim=3
        else:
            dim=2
        delta = int(args.poly_mask_size) #if args.dataset == 'cow-mra' else 64
        poly = Polynomial(map_size=delta, order=order_all, dim=dim, basis_type=args.basis_type)
    ...
    for epoch in range(start_epoch, config.config['epochs'] + 1):        #config.config['epochs']
        ...
        for batch_idx, batch_samples in enumerate(tqdm(dataloader_train, desc='Epoch {}'.format(epoch))):
            ...
            loss, images_dict, values_dict = process_batch_opti_process(args, config, batch_samples, poly=poly)
            ...
            for batch_idx, batch_samples in enumerate(tqdm(dataloader_val, desc='Epoch {}'.format(epoch))):
                ...
                if batch_idx < len(dataset_val):         # can specify how many samples you want to validation when training.
                    if not args.visual_euler:
                        with torch.no_grad():
                            loss_itn, images_dict, values_dict, = process_batch_opti_process(args, config, batch_samples, poly=poly)
                    else:
                        loss_itn, images_dict, values_dict, = process_batch_opti_process(args, config, batch_samples, poly=poly)
        ...
        for batch_idx, batch_samples in enumerate(tqdm(dataloader_test, desc='Epoch {}'.format(epoch))):
            ...
            if True:#batch_idx<=10:#True: #batch_idx in [1, 2]:
                if args.visual_euler:
                    ...
                    with torch.no_grad():
                        loss, images_dict, values_dict = process_batch_opti_process(args, config, batch_samples, poly)
