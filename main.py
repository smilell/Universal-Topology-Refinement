...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_softmax', default='./data/cremi_2d/test/softmax/', help='test labelmap path')      #'./data/cremi_2d/test/softmax/'
    ...
    parser.add_argument('--phase', default='test_pre', type=str,
                        choices=['train_pre', 'train_post', 'test', 'test_pre', 'test_post', 'euler_visualization', 'train_post_manualmask'])
    ...
    parser.add_argument('--inpaint_type', default='gaussian', type=str, choices=['gaussian', 'bw', 'gray','polynomial','polynomial_gauss', 'polynomial_binary'])
    ...
    parser.add_argument('--basis_type', default='legendre', type=str, choices=['legendre', 'chebyshev', 'hermite'])
    parser.add_argument('--order', default='6', type=str,)
    parser.add_argument('--poly_mask_size', default='64', type=str,)
    parser.add_argument('--test_synthetic_sample', default=False, action='store_true', help='input argmax into postprocessing')
    #-----------------------------------------------------------------------------------------------------------------
    ...
    if args.phase == 'train_pre':
        ...
        mk_dirs(os.path.join(args.out))
        euler(args)
    elif args.phase == 'train_post_manualmask':
        print(separator)
        print('Start to train segmentation model stage 2, model and tensorboard files will be saved in {} and {}'.format(args.model, args.TBout))
        print(separator)
        mk_dirs(args.TBout, args.model)
        ...
        if args.test_softmax is not None:
            args.isBetti = True
            args.trained_model_post = args.model
            args.out = args.TBout
            args.phase = 'test_post'
