...
def process_batch_opti_process(args, config, batch_samples, poly):
    ...
    if args.phase in ['train_pre', 'test_pre', 'train_patch', 'test_patch']:
        ...
        model_type = 'POST'
    elif args.phase in ['train_post_manualmask', 'test_post_manualmask']:
        model_type = 'POST_manualmask'
    ...
    # ------------------------------ 1. fetch data ------------------------------
    # if not model_type == 'POST_manualmask':
    ...
    if model_type == 'PRE':
        ...
        pred_softmax = c(pred)
    elif model_type == 'POST_manualmask':
        if config.model.training or args.test_synthetic_sample:
            labelmap_onehot_masked, mask, label_skel = random_mask(labelmap_onehot, inpaint_type=args.inpaint_type, poly=poly, var=2, order=int(args.order),map_size=int(args.poly_mask_size))
            if args.mode3d:
                post_input = smoothing_3d(labelmap_onehot_masked, kernel_size=11, sigma=0.2)
            else:
                post_input = smoothing_2d(labelmap_onehot_masked, kernel_size=5, sigma=0.5)
        else:           # test with real segmentations
            post_input = batch_samples['softmax_coarse'].to(config.device)
            mask = torch.ones_like(post_input)
            label_skel = torch.ones_like(post_input)

        pred = config.model(post_input)
        pred_softmax = c(pred)

        # post_input = (post_input >= 0.5).type(torch.float32).to(config.device)
        labelmap_onehot = labelmap_onehot
        labelmap_argmax = labelmap_onehot.squeeze(dim=1)
